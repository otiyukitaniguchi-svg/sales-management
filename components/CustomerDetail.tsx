...
export default function CustomerDetail() {
  ...
  return (
    <div className="flex-1 p-6 overflow-y-auto">
      {/* 架電履歴セクション - 上部に配置 */}
      <div className="rounded-2xl shadow-md border-2 border-black p-4 bg-gradient-to-br from-gray-50 to-white flex flex-col mb-6">
        <div className="flex items-center justify-between gap-2 mb-2">
          <div className="flex items-center gap-2">
            <span className="font-bold text-lg bg-gray-300 px-3 py-1 rounded-lg">架電履歴</span>
            <button onClick={handleCallStart} disabled={isCallActive}
              className="px-3 py-1 bg-blue-500 text-white rounded-lg text-sm font-semibold disabled:bg-gray-400 hover:bg-blue-600">開始</button>
            <button onClick={handleCallEnd} disabled={!isCallActive}
              className="px-3 py-1 bg-red-500 text-white rounded-lg text-sm font-semibold disabled:bg-gray-400 hover:bg-red-600">終了</button>
          </div>
          <div className="flex items-center gap-2 bg-yellow-50 border-2 border-black rounded-lg p-2">
            <label className="font-semibold text-sm text-gray-700 whitespace-nowrap">再コール日時:</label>
            <input type="date" value={recallDate} onChange={(e) => setRecallDate(e.target.value)} className="border-2 border-black px-2 py-1 rounded-lg text-sm" />
            <input type="time" value={recallTime} onChange={(e) => setRecallTime(e.target.value)} className="border-2 border-black px-2 py-1 rounded-lg text-sm" />
            <button onClick={() => alert(`再コール日時: ${recallDate} ${recallTime}`)} className="px-3 py-1 bg-yellow-500 text-white rounded-lg text-sm font-semibold hover:bg-yellow-600">設定</button>
          </div>
        </div>

        {isCallActive && (
          <div className="bg-blue-50 border-2 border-black rounded-lg p-3 mb-3">
            <div className="flex items-center gap-4 mb-3 pb-3 border-b-2 border-black">
              <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-4 gap-2 text-sm">
              <div>
                ...
                <input type="text" value={currentCall.operator || ''} onChange={(e) => setCurrentCall({ ...currentCall, operator: e.target.value })}
                  className="w-full border-2 border-black px-2 py-1 rounded-lg text-sm" />
              </div>
              <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse border-2 border-black rounded-lg overflow-hidden">
            <thead>
              <tr>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-16">担当者</th>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-20">対応日</th>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-16">開始</th>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-16">終了</th>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-20">対応者</th>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-16">性別</th>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-24">進捗</th>
                <th className="border-2 border-black px-4 py-2 text-left font-bold bg-gray-300 flex-1">コール履歴</th>
                <th className="border-2 border-black px-4 py-2 text-left whitespace-nowrap font-bold bg-gray-300 w-20">操作</th>
              </tr>
            </thead>
            <tbody>
              {callHistory.length === 0 ? (
                <tr><td colSpan={9} className="border-2 border-black px-4 py-3 text-center text-gray-400">履歴なし</td></tr>
              ) : (
                /* すべての履歴を表示（新しい順） */
                callHistory.map((entry, displayIndex) => {
                  ...
                  return (
                    <tr key={displayIndex} className={displayIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-16 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <input type="text" value={editingCallData?.operator || ''} onChange={(e) => handleEditingCallFieldChange('operator', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded" />
                        ) : (
                          entry.operator
                        )}
                      </td>
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-20 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <input type="text" value={editingCallData?.date || ''} onChange={(e) => handleEditingCallFieldChange('date', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded" />
                        ) : (
                          entry.date
                        )}
                      </td>
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-16 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <input type="text" value={editingCallData?.startTime || ''} onChange={(e) => handleEditingCallFieldChange('startTime', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded" />
                        ) : (
                          entry.startTime
                        )}
                      </td>
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-16 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <input type="text" value={editingCallData?.endTime || ''} onChange={(e) => handleEditingCallFieldChange('endTime', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded" />
                        ) : (
                          entry.endTime
                        )}
                      </td>
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-20 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <input type="text" value={editingCallData?.responder || ''} onChange={(e) => handleEditingCallFieldChange('responder', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded" />
                        ) : (
                          entry.responder
                        )}
                      </td>
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-16 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <select value={editingCallData?.gender || ''} onChange={(e) => handleEditingCallFieldChange('gender', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded">
                            <option value="">選択</option>
                            <option value="男性">男性</option>
                            <option value="女性">女性</option>
                          </select>
                      ...
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-24 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <select value={editingCallData?.progress || ''} onChange={(e) => handleEditingCallFieldChange('progress', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded">
                            <option value="">選択</option>
                            <option value="受注">受注</option>
                            <option value="見込みA">見込みA</option>
                            <option value="見込みC">見込みC</option>
                      ...
                      <td className="border-2 border-black px-4 py-2 cursor-pointer hover:bg-blue-100" onClick={() => handleEditCallHistory(displayIndex, entry)}>
                        {isEditing ? (
                          <input type="text" value={editingCallData?.note || ''} onChange={(e) => handleEditingCallFieldChange('note', e.target.value)} className="w-full border-2 border-black px-1 py-0.5 text-sm rounded" />
                        ) : (
                          entry.note
                        )}
                      </td>
                      <td className="border-2 border-black px-4 py-2 whitespace-nowrap w-40">
                        {isEditing ? (
                          <>
                            <button onClick={handleSaveCallHistory} className="px-2 py-1 bg-green-500 text-white rounded-lg text-xs mr-1 hover:bg-green-600">保存</button>
                            <button onClick={handleCancelCallHistory} className="px-2 py-1 bg-gray-400 text-white rounded-lg text-xs mr-1 hover:bg-gray-500">キャンセル</button>
                            <button onClick={() => handleDeleteCallHistory(displayIndex)} className="px-2 py-1 bg-red-500 text-white rounded-lg text-xs hover:bg-red-600">削除</button>
                          </>
                        ) : null
                      }</td>
                    </tr>
      ...
      {/* 顧客管理 */}
      <div className="rounded-2xl shadow-md border-2 border-black p-4 bg-gradient-to-br from-orange-50 to-white mb-6">
        ...
        <div className="flex gap-6 mt-4">
          ...
          <div className="flex-1 border-r-2 border-black">
            ...
            {/* フリガナ - ルビ形式 */}
            <div className="flex border-b-2 border-black">
              <div className="w-32 px-4 py-2 font-semibold text-lg text-gray-700 bg-gray-100 border-r-2 border-black whitespace-nowrap"></div>
